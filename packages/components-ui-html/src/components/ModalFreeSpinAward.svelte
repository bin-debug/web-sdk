...
<style lang="scss">
	.award {
		...
		width: min(30rem, calc(100vw - 1.5rem)); box-sizing: border-box; padding: clamp(22px, 5vw, 44px); border: 1px solid #d7dce3; border-radius: clamp(16px, 3vw, 24px);
	...
	.award__eyebrow { margin: 0; color: #354152; font-size: clamp(11px, 2.9vw, 13px); font-weight: 800; letter-spacing: .12em; text-transform: uppercase; }
	h2 { margin: .45rem 0 .5rem; color: var(--award-text); font-size: clamp(28px, 7.5vw, 38px); line-height: 1.1; }
	.award__intro { margin: 0 auto; max-width: 22rem; color: var(--award-muted); font-size: clamp(15px, 4vw, 18px); line-height: 1.45; }
	.award__spins { margin: 1.5rem 0; display: flex; align-items: baseline; justify-content: center; gap: .45rem; }
	.award__spins strong { color: var(--award-accent-strong); font-size: clamp(50px, 13vw, 60px); font-variant-numeric: tabular-nums; line-height: .9; }
	.award__spins span { color: var(--award-muted); font-size: clamp(16px, 4vw, 18px); font-weight: 700; }
	...
	.award__details dt { color: var(--award-muted); font-size: clamp(10px, 2.7vw, 12px); letter-spacing: .04em; text-transform: uppercase; }
	.award__details dd { margin: .3rem 0 0; color: var(--award-text); font-size: clamp(17px, 4.4vw, 20px); font-weight: 700; }
	.award__actions { margin-top: 1.5rem; display: grid; grid-template-columns: 1fr 1fr; gap: .7rem; }
	.award__button { min-height: clamp(48px, 13vw, 56px); border: 1px solid transparent; border-radius: .75rem; cursor: pointer; font: inherit; font-size: clamp(16px, 4vw, 18px); font-weight: 800; transition: transform .15s ease, filter .15s ease; }
	...
	@media (max-width: 500px) {
		.award { width: min(31rem, calc(100vw - 1.25rem)); padding: 30px 22px; }
	}
