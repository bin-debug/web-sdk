<script lang="ts">
	...
	const dismissIntro = () => {
		introVisible = false;
		if (stateFreeSpins.showAwardPopup && stateFreeSpins.allocations.length > 0) {
			stateModal.modal = { name: 'freeSpinAward' };
		}
	};

	// Authentication can finish while the branded intro is still on screen.
	// Keep its award out of the way until the player has explicitly continued.
	$effect(() => {
		if (introVisible && stateModal.modal?.name === 'freeSpinAward') stateModal.modal = null;
	});
</script>
...
{#if !introVisible && !bonusModalOpen}
	<img class="game-logo" src="/assets/sprites/game/logo.png" alt="Skull Fiesta Frenzy" />
{/if}
