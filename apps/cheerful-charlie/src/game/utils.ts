...
export const getSymbolInfo = ({
	...
	// [builder] Flatten: always render a static sprite, never Spine.
	const info = SYMBOL_INFO_MAP[symbolKey] as Record<string, any>;
	for (const st of ['static', 'postWinStatic', 'spin', 'land', 'win']) {
		if (info[st] && info[st].type === 'sprite') return info[st];
	}
	return info[state];
};
