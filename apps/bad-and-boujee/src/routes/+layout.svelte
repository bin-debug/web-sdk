<script lang="ts">
	...
	const introFeatures = [{"icon":"👥","title":"Street Reels","subtitle":"Spin the city block"},{"icon":"🎁","title":"Free Spins","subtitle":"Land Scatters to trigger the bonus"},{"icon":"💰","title":"5,000× Max Win","subtitle":"Chase the biggest score"}];
	...
	// Ticker + game/studio name bar hide while the Buy Bonus screen is open, and
	// come straight back the instant it closes — driven off stateModal directly
	// (same pattern as the mobile bet bar), so there's no separate restore step.
...
{#if !introVisible && !bonusModalOpen}
	<div class="top-bar">
		<div class="tb-left">
			...
			<span class="tb-name">Bad And Boujee</span>
		</div>
