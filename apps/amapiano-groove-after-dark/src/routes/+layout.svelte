<script lang="ts">
	...
	const introFeatures = [{"icon":"*","title":"Cluster Pays","subtitle":"Match 5+ adjacent symbols to win"},{"icon":"*","title":"Free Spins","subtitle":"Land Scatters to trigger the bonus round"},{"icon":"*","title":"5 000x Max Win","subtitle":"Chase the ultimate top prize"}];
...
{#if !introVisible && !bonusModalOpen}
	<div class="top-bar" class:top-bar--no-ticker={stateConfig.jurisdiction.disabledNoticeBar}>
		<div class="tb-left">
			...
			<span class="tb-name">Amapiano Groove After Dark</span>
		</div>
...
{#if !introVisible && !bonusModalOpen}
	<img class="game-logo" src="/assets/sprites/game/logo.webp" alt="Amapiano Groove After Dark" />
{/if}
